...
export default function MapComponent({
  ...
  useEffect(() => {
    ...
    // Center on Nainital, Uttarakhand, India (Nainital: 29.38, 79.45)
      ...
      if (existingLayer) {
        ...
        try {
          ...
          existingLayer.setStyle({
            ...
            fillColor: isHollow ? "transparent" : (layerConf.fillColor || layerConf.color),
  ...
  // Map Controls: Pan to Nainital bounds safely
  const handleZoomToDistrict = () => {
    ...
    // Fallback static center of Nainital
    map.setView([29.38, 79.45], 10, { animate: true });
  };
        ...
        <div className="relative flex items-center bg-white/95 dark:bg-slate-900/95 backdrop-blur border border-slate-200 dark:border-slate-800 rounded-lg shadow-md transition-shadow duration-200 focus-within:shadow-lg focus-within:border-indigo-400 dark:focus-within:border-indigo-500">
          ...
          {mapSearchQuery ? (
            <button
              ...
              className="absolute right-2.5 p-1 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-200 rounded-md hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
            >
            ...
            {filteredSearchFeatures.map((feat) => {
              ...
              return (
                <button
                  ...
                  className="w-full text-left px-3.5 py-2.5 hover:bg-indigo-50/70 dark:hover:bg-indigo-950/70 active:bg-indigo-100 text-xs text-slate-700 dark:text-slate-200 font-medium transition-colors flex items-center justify-between gap-1 border-none bg-transparent cursor-pointer"
                >
